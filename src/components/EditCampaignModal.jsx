...
const EditCampaignModal = ({ isOpen, onClose, onSubmit, campaign }) => {
  ...
  useEffect(() => {
    if (campaign) {
      setFormData({
        ...
        recipients: campaign.recipientType || 'All Members',
  ...
  const handleSubmit = (e, saveType) => {
    e.preventDefault();
    onSubmit({ ...formData, saveType, id: campaign.id });
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#1e2a3a] rounded-lg shadow-2xl w-full max-w-xl border border-slate-700">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-slate-700">
          <h2 className="text-white text-lg font-semibold">Edit Campaign - {campaign.id}</h2>
          <button
            onClick={onClose}
