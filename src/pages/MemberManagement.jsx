...
const MemberManagement = () => {
  ...
  const handleAddMember = async (newMemberData) => {
    try {
      ...
      const memberPayload = {
        ...
        talentScout: newMemberData.talentScout,
  ...
  const handleEditMember = async (updatedData) => {
    try {
      ...
      const memberPayload = {
        ...
        talentScout: updatedData.talentScout,
  ...
  const handleDeleteMember = async (memberId, memberName) => {
    const confirmed = await confirm({
      title: 'Delete Member',
      message: `Deleting ${memberName} will ALSO delete the linked Onboarding record (cascade). This is to keep Member Management and Onboarding in sync. This action cannot be undone. Continue?`,
    ...
    if (confirmed) {
      try {
        ...
        if (response.success) {
          await fetchMembers();
          const cascaded = response.cascadedOnboardings || 0;
          toast.success(
            cascaded > 0
              ? `Member deleted. ${cascaded} linked onboarding${cascaded > 1 ? 's' : ''} also removed.`
